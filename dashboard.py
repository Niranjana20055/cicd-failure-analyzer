...
.block-container { padding: 0 40px 40px !important; max-width: 100% !important; }
...
.metric-top { height: 2px; margin: -18px -20px 16px; border-radius: 2px 2px 0 0; }
.metric-label {
...
.card:hover { border-color: rgba(99,70,245,0.2); }
...
.feed-item:hover { background: rgba(255,255,255,0.04); border-color: rgba(99,70,245,0.15); }
.feed-icon {
    width: 36px; height: 36px; flex-shrink: 0;
    background: rgba(99,70,245,0.12);
    border-radius: 9px;
    display: flex; align-items: center; justify-content: center;
    font-size: 17px;
}
.feed-repo { font-size: 13px; font-weight: 600; color: #CBD5E1; margin-bottom: 3px; }
.feed-meta { font-size: 11px; color: #334155; }
.feed-badge {
...
.empty-sub { font-size: 14px; color: #334155; line-height: 1.6; max-width: 400px; margin: 0 auto; }
...
.stExpander summary { color: #818CF8 !important; font-size: 13px !important; font-weight: 500 !important; }
...
<div style="padding:8px 0 32px;">
    <div style="font-size:11px;font-weight:600;color:#6346F5;
    text-transform:uppercase;letter-spacing:0.08em;margin-bottom:12px;
    display:flex;align-items:center;gap:6px;">
        <span>⚡</span> AI-Powered DevOps Intelligence Platform
    </div>
    <div style="font-size:34px;font-weight:700;color:#F8FAFC;
    ...
    <div style="font-size:14px;color:#475569;max-width:560px;line-height:1.6;">
        ...
        Automated PR comments · Real-time observability dashboard
    </div>
</div>
<div style="height:1px;background:linear-gradient(90deg,transparent,
rgba(99,70,245,0.2),transparent);margin-bottom:28px;"></div>
""", unsafe_allow_html=True)
...
if not logs:
    ...
    <div class="empty-state">
        ...
        <div class="empty-title">No failures analyzed yet</div>
        <div class="empty-sub">
            Connect your GitHub repository via webhook and trigger a
            workflow failure — the AI agent will automatically diagnose
            it and populate this dashboard in real time.
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    border-radius:12px;padding:20px 24px;max-width:600px;margin:0 auto;">
        <div style="font-size:13px;font-weight:600;color:#818CF8;margin-bottom:12px;">
            🚀 Quick Start
        </div>
        <div style="font-size:12px;color:#64748B;line-height:2;font-family:'JetBrains Mono',monospace;">
            1. Start server: <span style="color:#38BDF8;">uvicorn main:app --reload</span><br>
            2. Expose locally: <span style="color:#38BDF8;">ngrok http 8000</span><br>
            3. Add webhook to your GitHub repo<br>
            4. Trigger a workflow failure<br>
            5. Watch this dashboard populate automatically
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="metric-grid">
        <div class="metric-card">
            <div class="metric-top" style="background:linear-gradient(90deg,#6346F5,#818CF8);"></div>
            <div class="metric-label">Failures Analyzed</div>
            <div class="metric-val">{total}</div>
            <div class="metric-sub">Total pipeline failures processed</div>
        </div>
        <div class="metric-card">
            <div class="metric-top" style="background:linear-gradient(90deg,#34D399,#6EE7B7);"></div>
            <div class="metric-label">High Confidence</div>
            <div class="metric-val" style="color:#34D399;">{high_conf}</div>
            <div class="metric-sub">Accurate root cause diagnoses</div>
        </div>
        <div class="metric-card">
            <div class="metric-top" style="background:linear-gradient(90deg,#38BDF8,#7DD3FC);"></div>
        ...
        <div class="metric-card">
            <div class="metric-top" style="background:linear-gradient(90deg,#FBBF24,#FDE68A);"></div>
            ...
            <div class="metric-sub">Most recurring failure pattern</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with tab1:
        ...
        with col2:
            ...
            fig_conf = go.Figure(go.Pie(
                ...
                textinfo="none",
                hovertemplate="<b>%{label}</b><br>%{value} diagnoses (%{percent})<extra></extra>"
            ))
            ...
            fig_conf.update_layout(
                title=dict(text="Diagnosis Confidence Distribution",
                          font=dict(color="#F1F5F9", size=14, family="Inter")),
                paper_bgcolor="rgba(0,0,0,0)",
                showlegend=True,
                legend=dict(
        ...
        with col4:
            ...
            fig_pr = go.Figure(go.Bar(
                ...
                y=pr_data["count"],
                marker=dict(
                    color=["#34D399" if s == "Yes" else "#F87171"
                           for s in pr_data["status"]],
                    line=dict(color="rgba(0,0,0,0)", width=0)
                ),
                hovertemplate="<b>%{x}</b><br>%{y} failures<extra></extra>"
            ))
            fig_pr.update_layout(
                title=dict(text="PR Comment Status",
                          font=dict(color="#F1F5F9", size=14, family="Inter")),
                paper_bgcolor="rgba(0,0,0,0)",
                plot_bgcolor="rgba(0,0,0,0)",
                font=dict(family="Inter", color="#64748B", size=11),
    ...
    with tab2:
        ...
        for _, row in df.iterrows():
            ...
            with st.expander(
                f"{emoji}  {row['repo']} — {row['workflow']} — "
                f"{cat_display} — {ts_str}"
            ):
                ...
                with col_a:
                    ...
                    <div style="margin-bottom:16px;">
                        ...
                        letter-spacing:0.06em;margin-bottom:4px;">Repository</div>
                        <div style="font-size:13px;color:#CBD5E1;font-family:monospace;">
                        {row['repo']}</div>
                    </div>
                    <div style="margin-bottom:16px;">
                        <div style="font-size:11px;color:#475569;text-transform:uppercase;
                        letter-spacing:0.06em;margin-bottom:4px;">Workflow</div>
                        <div style="font-size:13px;color:#CBD5E1;">{row['workflow']}</div>
                    </div>
                    <div>
                        <div style="font-size:11px;color:#475569;text-transform:uppercase;
                        letter-spacing:0.06em;margin-bottom:4px;">Branch · Commit</div>
                        <div style="font-size:13px;color:#CBD5E1;font-family:monospace;">
                        {row['branch']} · {sha}</div>
                    </div>
                    """, unsafe_allow_html=True)
                ...
                with col_b:
                    ...
                    <div style="margin-bottom:16px;">
                        <div style="font-size:11px;color:#475569;text-transform:uppercase;
                        letter-spacing:0.06em;margin-bottom:4px;">Failure Category</div>
                        <div style="font-size:13px;color:#818CF8;">{cat_display}</div>
                    </div>
                    <div style="margin-bottom:16px;">
                        <div style="font-size:11px;color:#475569;text-transform:uppercase;
                        letter-spacing:0.06em;margin-bottom:4px;">Confidence</div>
                        <div style="font-size:13px;font-weight:600;color:{conf_color};">
                        {row['confidence'].upper()}</div>
                    </div>
                    <div>
                        <div style="font-size:11px;color:#475569;text-transform:uppercase;
                        letter-spacing:0.06em;margin-bottom:4px;">PR Comment</div>
                        <div style="font-size:13px;color:#CBD5E1;">{row['pr_comment_posted']}</div>
                    </div>
                    """, unsafe_allow_html=True)

                st.markdown(f"""
                <div style="background:rgba(99,70,245,0.05);
                border:1px solid rgba(99,70,245,0.15);
                border-left:3px solid #6346F5;
                border-radius:0 10px 10px 0;
                padding:14px 16px;margin-top:12px;margin-bottom:10px;">
                    <div style="font-size:11px;font-weight:600;color:#6346F5;
                    text-transform:uppercase;letter-spacing:0.06em;margin-bottom:8px;">
                        🔍 Root Cause
                    </div>
                ...
                border-radius:0 10px 10px 0;
                padding:14px 16px;">
                    <div style="font-size:11px;font-weight:600;color:#34D399;
                    text-transform:uppercase;letter-spacing:0.06em;margin-bottom:8px;">
                        🛠️ Suggested Fix
                    </div>
                    <div style="font-size:13px;color:#CBD5E1;line-height:1.7;">
                    {row['suggested_fix']}</div>
                </div>
                """, unsafe_allow_html=True)

    # ════════════════════════════════════════
    # TAB 3 — FULL LOG TABLE
    # ════════════════════════════════════════
    with tab3:
        st.markdown("""
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">
            ...
            border:1px solid rgba(99,70,245,0.2);border-radius:4px;padding:3px 9px;">
                Auto-updated
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        display_df = df[[
            "repo", "workflow", "branch", "commit_sha",
            "failure_category", "confidence", "pr_comment_posted", "timestamp"
        ]].copy()
        ...
        display_df["Failure Category"] = display_df["Failure Category"].str.replace(
            "_", " "
        ).str.title()
...
border-top:1px solid rgba(255,255,255,0.04);margin-top:32px;">
